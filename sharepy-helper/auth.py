...
class SharePointADFS(requests.auth.AuthBase):
    ...
    def _get_token(self):
        ...
        try:
            ...
            print("Token request failed. The server did not send a valid response\r")
            return
        try:
            samlAssertion = (re.search(r'<saml:Assertion.*\/saml:Assertion>', response.text)).group(0)
        except AttributeError:
            print("Token requested, but response did not include the STS SAML Assertion.\r")
            return
        ...
        try:
            ...
            print("BinarySecurityToken request failed. The server did not send a valid response\r")
            return
        try:
            binarySecurityToken = (re.search(r'BinarySecurityToken Id.*>([^<]+)', response.text)).group(1)
        except AttributeError:
            print("BinarySecurityToken requested, but response did not include the STS SAML Assertion.\r")
            return
